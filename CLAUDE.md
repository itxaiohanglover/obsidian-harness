...
当 vault 操作涉及搜索、读取、写入笔记时：
1. **优先使用 obsidian-cli**（需要 Obsidian 在后台运行）：
   - 搜索：`obsidian search query="..."` — 走 Obsidian 原生搜索引擎，比 grep 准
   - 日记追加：`obsidian daily:append content="..."` — 不用手动定位文件
   - 反向链接：`obsidian backlinks file="..."` — 无需全库正则
   - 属性操作：`obsidian property:set name=... value=...` — 不用手动解析 YAML
2. **回退到直接文件操作**：如果 obsidian-cli 报错（Obsidian 未运行），使用标准文件读写
3. **不要混用**：一个操作内选择一种方式，不要先 cli 读再直接写
